...
def worker_process_queue():
    while True:
        ...
        print("pending queue", form)
        output = handle_submit(form, files)
        print("output", output)
        socketio.emit('new_file', {'output_filename': os.path.basename(output),'audioID':form.get('audioID')})
...
def save_output(output, sample_rate, text):
    ...
    audio_write(
        output_filename, output.squeeze(), sample_rate, add_suffix=False) # loudness_compressor=True strategy="loudness", loudness_headroom_db=16, 
    return output_filename
        
def handle_submit(form, files):
    model = form.get("model")
    prompt = form.get("text")
    model_parameters = {
        "duration": form.get("duration"),
        "top_k": form.get("topk"),
        "top_p": form.get("topp"),
        "temperature": form.get("temperature"),
        "cfg_coef": form.get("cfg_coef"),
    }
...
def get_audio_files():
    ...
    limited_audio_files = audio_files_dicts[:10]  # Limit to 10 items
    # print("jsonify(audio_files_dicts)", (jsonify(audio_files_dicts))[slice(10)])
    return (jsonify(limited_audio_files))

@app.route('/', methods=['GET', 'POST'])
def home_and_submit():
    form = MusicForm(request.form)
    # print("DATA", request.get_json(silent=True).get("data"))
    output_filename = None  # Initialize output_filename here

    if request.method == 'POST': # and form.validate():
        form = request.get_json(silent=True).get("data")
        files = None
